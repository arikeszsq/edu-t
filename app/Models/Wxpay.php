...
class WxPay
{
    ...
    function __construct($openid, $out_trade_no, $body, $total_fee, $profit_sharing = 'N', $shops = [])
    {
        ...
        $this->total_fee = $total_fee;//金额:使用分为单位的所有要乘100
    ...
    private function unifiedorder()
    {
        ...
        if ($this->profit_sharing == 'Y') {
            ...
            $parameters = array(
                ...
                'total_fee' => $this->total_fee, //总金额 单位 分,因为充值金额最小是1 而且单位为分 如果是充值1元所以这里需要*100
                'spbill_create_ip' => $_SERVER['REMOTE_ADDR'], //终端IP,服务器IP
                'notify_url' => 'https://*****.com/api/pay/notify', //支付成功回调地址，注意1. 确保外网能正常访问 2.https
