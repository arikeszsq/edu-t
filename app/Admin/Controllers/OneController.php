...
class OneController extends ActivityController
{
    ...
    protected function grid()
    {
        return Grid::make(new Activity(), function (Grid $grid) {
            ...
            $grid->actions(function (Grid\Displayers\Actions $actions) {
                ...
                $actions->append('<a class="btn btn-sm btn-success" style="margin: 3px;;" href="/admin/form-fields?activity_id='.$actions->row->id.'">表单设置</a>');
    ...
    protected function form()
    {
        return Form::make(new Activity(), function (Form $form) {
            $form->display('id');
            $form->multipleImage('bg_banner', '轮播图')
                ->required()
                ->saveFullUrl()
                ->autoUpload()
                ->saving(function ($v) {
